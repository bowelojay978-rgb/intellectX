...
| `/admin` | Placeholder, staff-only | Not yet implemented | Runtime guard fails closed unless trusted Clerk claims resolve to `admin`; Convex admin mutations also fail closed without trusted role claims in Convex identity. | Placeholder route only; must not expose privileged management actions to denied users. | Build coverage, staff route policy/runtime unit tests, Convex RBAC unit tests |
| `/admin/course-review` | Placeholder, staff-only | Not yet implemented | Runtime guard fails closed unless trusted Clerk claims resolve to `admin`; Convex review/publish/archive mutations are admin-only. | Placeholder route only; dashboard UI is still not implemented. | Build coverage, staff route policy/runtime unit tests, Convex workflow unit tests |
| `/admin/instructors` | Placeholder, staff-only | Not yet implemented | Runtime guard fails closed unless trusted Clerk claims resolve to `admin`; instructor-management mutations are not implemented. | Placeholder route only; must not expose instructor-management actions to denied users. | Build coverage, staff route policy/runtime unit tests |
| `/instructor` | Placeholder, staff-only | Not yet implemented | Runtime guard fails closed unless trusted Clerk claims resolve to `instructor` or `admin`; Convex draft/submit mutations also require trusted instructor/admin role claims. | Placeholder route only; must not expose course-management UI yet. | Build coverage, staff route policy/runtime unit tests, Convex RBAC unit tests |
| `/instructor/courses` | Placeholder, staff-only | Not yet implemented | Runtime guard fails closed unless trusted Clerk claims resolve to `instructor` or `admin`; instructors can only submit their own courses server-side. | Placeholder route only; dashboard UI is still not implemented. | Build coverage, staff route policy/runtime unit tests, Convex workflow unit tests |
| `/instructor/courses/new` | Placeholder, staff-only | Not yet implemented | Runtime guard fails closed unless trusted Clerk claims resolve to `instructor` or `admin`; Convex draft creation sets instructor ownership from trusted identity. | Placeholder route only; create-course UI is still not implemented. | Build coverage, staff route policy/runtime unit tests, Convex workflow unit tests |
...
| `courses.createInstructorCourseDraft`, `courses.submitCourseForReview` | Staff workflow mutation | Convex `ctx.auth.getUserIdentity()` trusted role claim plus token identifier | Missing/malformed/learner role claims fail closed. Instructors can create drafts and submit only their own courses; admins can create/submit. Hidden states remain invisible to learner queries. | Convex RBAC and workflow unit tests |
| `courses.requestCourseChanges`, `courses.approveCourse`, `courses.publishCourse`, `courses.unpublishCourse`, `courses.archiveCourse` | Admin workflow mutation | Convex `ctx.auth.getUserIdentity()` trusted role claim plus token identifier | Missing/malformed/non-admin role claims fail closed. Admin workflow actions write audit logs and learner visibility remains approved plus published only. | Convex RBAC and workflow unit tests |
| `auditLogs` | Internal audit table | Server-side workflow mutations only | Append-only workflow audit foundation; not exposed publicly to learners. | Convex workflow unit tests |
